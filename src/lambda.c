...
/* constants/macros ----------------------------------------------------------*/
#define LOOPMAX     30000           /* maximum count of search loop */
...
/* modified lambda (mlambda) search (ref. [2]) -------------------------------*/
extern int msearch(int n, int m, int ncand, const double *L, const double *D,
	               const double *zs , double *za, double *Dza, double *s, double *sw) 
{
	int i, j, k, c, nn = 0, imax = 0;
	double newdist, maxdist = 1E99, y, sumweight;
	double *S = zeros(n, n), *dist = mat(n, 1), *zb = mat(n, 1), *z = mat(n, 1), *step = mat(n, 1);
	double *zcand = mat(ncand*n, 1);
	double *scand = mat(ncand, 1);
	...
	for (i = 0; i < n; i++)
	{
		...
		for (j = 0; j < ncand; j++)
		{
			Dza[i] = Dza[i] + (zcand[i + j * n] - zs[i])*(zcand[i + j * n] - zs[i])* exp(-0.5*scand[j] * D[0]);
		}
		Dza[i] = Dza[i] / sumweight;
	}
	...
	{
		double y = za[i] - zs[i];
		sw[0] = sw[0] + y * y / (D[i]/D[0]);
	}

	for (i = 0; i < m; i++)
		s[i] = scand[i];

	free(S); free(dist); free(zb); free(z); free(step); free(scand); free(zcand);
...
extern int lambda_reduction(int n, const double *a, const double *Q, double *L, double *Z, double *D, double *z)
{
	int info;
