...
extern int rtkoutstat(rtk_t *rtk, char *buff)
{
	...
	if (est&&rtk->opt.ionoopt==IONOOPT_UC12) {
        for (i=0;i<MAXSAT;i++) {
            ssat=rtk->ssat+i;
            if (!ssat->vs) continue;
            satno2id(i+1,id);
                     ...
                     const prcopt_t *opt)
{
	...
	return 2.0*(opt->ionoopt==IONOOPT_IF12?3.0:1.0)*(a*a+b*b/sinel/sinel+c*c)+d*d;
}
/* baseline length -----------------------------------------------------------*/
...
static void detslp_ll(rtk_t *rtk, const obsd_t *obs, int i, int rcv)
{
	...
	int f,sat=obs[i].sat,prn,sys=satsys(sat,&prn);
    ...
    for (f=0;f<rtk->opt.nf;f++) {
        ...
        if (rtk->tt>=0.0) { /* forward */
            if (obs[i].LLI[f]&1) {
                errmsg(rtk,"slip detected forward  (sat=%c%2d rcv=%d F=%d LLI=%x)\n",
					   sys2char(sys),prn,rcv,f+1,obs[i].LLI[f]);
			}
			slip=obs[i].LLI[f];
		}
		else { /* backward */
			if (LLI&1) {
				errmsg(rtk,"slip detected backward (sat=%c%2d rcv=%d F=%d LLI=%x)\n",
					   sys2char(sys),prn,rcv,f+1,LLI);
            }
            slip=LLI;
        }
        /* detect slip by parity unknown flag transition in LLI */
        if (((LLI&2)&&!(obs[i].LLI[f]&2))||(!(LLI&2)&&(obs[i].LLI[f]&2))) {
			errmsg(rtk,"slip detected half-cyc (sat=%c%2d rcv=%d F=%d LLI=%x->%x)\n",
                   sys2char(sys),prn,rcv,f+1,LLI,obs[i].LLI[f]);
            slip|=1;
        }
        /* save current LLI */
...
static void detslp_gf_L1L2(rtk_t *rtk, const obsd_t *obs, int i, int j,
                           const nav_t *nav)
{
    int sat=obs[i].sat,prn,sys=satsys(sat,&prn);
    double g0,g1;
    ...
    if (g0!=0.0&&fabs(g1-g0)>rtk->opt.thresslip) {
        ...
        errmsg(rtk,"slip detected GF2_jump (sat=%c%2d GF_L1_L2=%.3f %.3f)\n",sys2char(sys),prn,g0,g1);
    }
}
                           ...
                           const nav_t *nav)
{
    ...
    if (g0!=0.0&&fabs(g1-g0)>rtk->opt.thresslip) {
		...
		errmsg(rtk,"slip detected GF5_jump (sat=%c%2d GF_L1_L5=%.3f %.3f)\n",sys2char(sys),prn,g0,g1);
    }
}
                       ...
                       const nav_t *nav)
{
    ...
    for (f=0;f<rtk->opt.nf;f++) {
		...
		errmsg(rtk,"slip detected (sat=%c%2d rcv=%d L%d=%.3f %.3f thres=%.3f)\n",
               sys2char(sys),prn,rcv,f+1,dph,dpt,thres);
    }
#endif
}
                   ...
                   const int *iu, const int *ir, int ns, const nav_t *nav)
{
    ...
    for (f=0;f<nf;f++) {
        ...
        for (i=0;i<ns;i++) {
			...
			if (rtk->opt.ionoopt==IONOOPT_IF12) slip|=rtk->ssat[sat[i]-1].slip[1];
                      ...
                      const prcopt_t *opt, double *y)
{
	...
	if (opt->ionoopt==IONOOPT_IF12) { /* iono-free linear combination */
                 ...
                 int index, double *y, double *e, double *azel)
{
	...
	for (i=0;i<n;i++) {
		...
		trace(4,"sat=%c%2d %13.3f %13.3f %13.3f %13.10f %6.1f %5.1f\n",
			  sys2char(sys),prn,rs[i*6],rs[1+i*6],rs[2+i*6],dts[i*2],azel[i*2]*R2D,
              azel[1+i*2]*R2D);
    }
                 ...
                 double *H, double *R, int *vflg)
{
    ...
    int i,j,k,m,f,ff,nv=0,nb[NFREQ*4*2+2]={0},b=0,sysi,sysj,nf=NF(opt),prn1,prn2,sys1,sys2;
    ...
    for (f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
        ...
        for (j=0;j<ns;j++) {
            ...
            if (opt->maxinno>0.0&&fabs(v[nv])>opt->maxinno) {
				...
				sys1=satsys(sat[i],&prn1);
				sys2=satsys(sat[j],&prn2);
				errmsg(rtk,"outlier rejected (sat=%c%02d-%c%02d %s%d v=%.3f)\n",
					   sys2char(sys1),prn1,sys2char(sys2),prn2,f<nf?"L":"P",f%nf+1,v[nv]);
                continue;
            }
			...
			trace(4,"sat=%c%02d-%c%02d %s%d v=%13.3f R=%8.6f %8.6f\n",
				sys2char(sys1),prn1,sys2char(sys2),prn2,f<nf?"L":"P",f%nf+1,v[nv],Ri[nv],Rj[nv]);
                  ...
                  int nv, double thres)
{
	...
	int i,stat=1,sat1,sat2,type,freq,prn1,prn2,sys1,sys2;
	char *stype;
	...
	for (i=0;i<nv;i++) {
		...
		sat1=(vflg[i]>>16)&0xFF;
		sat2=(vflg[i]>> 8)&0xFF;
		sys1=satsys(sat1,&prn1);
		sys2=satsys(sat2,&prn2);
		type=(vflg[i]>> 4)&0xF;
		freq=vflg[i]&0xF;
		stype=type==0?"L":(type==1?"L":"C");
		errmsg(rtk,"large residual (sat=%c%2d-%c%2d %s%d v=%6.3f sig=%.3f)\n",
			  sys2char(sys1),prn1,sys2char(sys2),prn2,stype,freq+1,v[i],SQRT(R[i+i*nv]));
    }
                  ...
                  const nav_t *nav)
{
	...
	int nf=opt->ionoopt==IONOOPT_IF12?1:opt->nf;
...
extern void rtkinit(rtk_t *rtk, const prcopt_t *opt)
{
	...
	rtk->xw = zeros(rtk->na, 1);
	rtk->Pw = zeros(rtk->na, rtk->na);
    rtk->nfix=rtk->neb=0;
	for (i=0;i<MAXSAT;i++)
	{
        rtk->ambc[i]=ambc0;
...
extern int rtkpos(rtk_t *rtk, const obsd_t *obs, int n, const nav_t *nav)
{
...
#ifdef _OUTPUT_DD_
	int ir[40] = { 0 }, iu[40] = { 0 }, ns = 0, j = 0, sat[40] = { 0 }, sys[40] = { 0 }, flag[40] = { 0 }, frq[2] = { 0 };
	double resP[40][3] = { 0 };
	double resL[40][3] = { 0 };
	double curP[3] = { 0 }, curL[3] = { 0 };
	int wn = 0;
	double ws = time2gpst(obs[0].time, &wn);
	double age = 0.0;
	...
	FILE *fobs = (rtk->sol.time.time == 0) ? (fopen("obs.txt", "w")) : (fopen("obs.txt", "a"));
	FILE *fDD_G = (rtk->sol.time.time == 0) ? (fopen("dd_G.txt", "w")) : (fopen("dd_G.txt", "a"));
	...
	sprintf(blqf,"%s%s",buf,"\\ocnload.blq\0");

	if (rtk->opt.odisp[0][0]==0.0)
	{
		readblq_pos(blqf,rtk->sol.rr,rtk->opt.odisp[0]);
	}
	...
	if (rtk->opt.prcOpt_Ex.navSys&SYS_CMP)
	{
